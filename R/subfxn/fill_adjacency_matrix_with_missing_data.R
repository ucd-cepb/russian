...
fill_adjacency_matrix_with_missing_data <- function(el, missing_data, self=FALSE, el_directed=FALSE){
  ...
  if(dim(el)[2] == 3){
    ...
    A <- matrix(data=0, nrow=length(all_a), ncol=length(all_a))
    row.names(A) <- all_a
    colnames(A) <- all_a
    ...
    if(el_directed){
      ...
      for(i in 1:length(all_a)){
        for(j in 1:length(all_a)){
          if(i!=j | (i==j & self)){
            ## subset edgelist data frame for edges to / from actor i -- actor j
            d <- filter(el, from==all_a[i] & to==all_a[j] | from==all_a[j] & to==all_a[i])
            ## get unique weights
            r_tmp <- unique(d$r)
            ## if there is a single weight or this edge is not in the edgelist
            if(length(r_tmp)==1 | is_empty(r_tmp)){
              if(is_empty(r_tmp)){r_tmp <- 0}  # if no edge in edgelist, set entry to 0
              ### assign matrix entry
              A[all_a[i],all_a[j]] <- r_tmp
              A[all_a[j],all_a[i]] <- r_tmp
              
            } else{
              ## if there are multiple unique weights but only one is non-zero
              if(0 %in% r_tmp){
                r_tmp <- max(r_tmp, na.rm=TRUE) # set entry to max weight (i.e., non-zero weight)
                A[all_a[i],all_a[j]] <- r_tmp
                A[all_a[j],all_a[i]] <- r_tmp
              } else if(!(0 %in% r_tmp)){
                ## if there are multiple unique weights
    ...
    # pivot back to edgelist.
    A2 <- A
    
    diag(A2) <- NA
    
    if(sum( A2[which(rownames(A2) %in% missing_data),which(colnames(A2) %in% missing_data)], na.rm=TRUE) == 0){
      A2[which(rownames(A2) %in% missing_data),which(colnames(A2) %in% missing_data)] <- NA
    } else{
      stop('ERROR: There is a non-zero weight between two un-surveyed organizations.')
    }
