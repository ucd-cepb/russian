...
2. What is the variation in responses on level of involvement in kelp forest issues – primarily, routinely, rarely <-- usefulness of exploring existing / potential fit
...
interact.net <- read_csv(here('confidential_data','intermediate','kelp_by_org_by_interact_2025-02-05.csv')) 
...
What is the **response rate?**
```{r}
length(unique(interact.net$response_id[which(!is.na(interact.net$interact_type))]))/length(unique(interact.net$response_id))
```
...
```{r}
interact.net %>%
  filter(!is.na(interact_type) & interact_type != 'Other:') %>%
...
Are certain interaction types linked to ways in which people get kelp info?
